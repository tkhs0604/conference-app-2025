package io.github.droidkaigi.confsched.testing.behavior
...
sealed class TestNode<T> {
    ...
    data class Run<T>(
        val action: suspend context(ComposeUiTest) T.() -> Unit,
    ) : TestNode<T>()
...
class TestCaseTreeBuilder<T> {
    ...
    fun doIt(action: suspend context(ComposeUiTest) T.() -> Unit) {
...
private fun <T> collectCheckNodes(root: TestNode.Describe<T>): List<CheckNode<T>> {
    ...
    fun traverse(node: TestNode<T>, parentDescription: String, ancestry: List<AncestryNode<T>>) {
        when (node) {
            is TestNode.Describe -> {
                val currentDescription = if (parentDescription.isEmpty()) node.description else "$parentDescription - ${node.description}"
